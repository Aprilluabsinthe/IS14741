...
# modify here
char_deal = 16
pn_value = "22757365726e616d65223a20226775"
filename = "try_last4_16.txt"
...
cookie_IV_origin = "5468697320697320616e204956343536"
...
for i in range(hex_len):
    ...
    last_nchar = cookie_IV_origin[32-(char_deal*2):]
    cookie_IV = cookie_IV_origin[:32-(char_deal*2)] + sxor(last_nchar, guess_xor_pad).zfill(char_deal*2)
    print  "\noriginal C_[IV][m]: ", last_nchar, "  guess: ", guess, "  Xored: ", guess_xor_pad
    print "Xored Cookie C_[IV][%d]: ", sxor(cookie_IV_origin[32-(char_deal*2):], guess_xor_pad).zfill(char_deal*2)
    cookie = cookie_IV + cookie_c0
    ...
    with open(filename, 'a+') as f:
        ...
        f.write("\nXored Cookie C_[IV][%d]: ")
        f.write(sxor(cookie_IV_origin[32-(char_deal*2):], guess_xor_pad).zfill(char_deal*2))
    conn = remote(host, port)
...
if get_guess == null:
    ...
    print "get xored cookie_IV", get_guess
    with open(filename, 'a+') as f:
        f.write("\n get xored cookie_IV")
        f.write(cookie_c1)
